...
const SUPPORTED_HEADERS: Array<{ code: ParameterCode; test: (header: string, token: string) => boolean }> = [
  ...
  { code: "gmax", test: (_header, token) => token === "gmax" },
  { code: "vs", test: (header, token) => token === "vs" || header.includes("v_s") },
  { code: "eoed", test: (_header, token) => token === "eoed" },
  { code: "mv", test: (_header, token) => token === "mv" },
